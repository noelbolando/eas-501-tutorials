# Tutorials for EAS 501.074 - Sustainable Urban Systems

<img width="641" height="480" alt="Screenshot 2026-04-02 at 12 44 55 PM" src="https://github.com/user-attachments/assets/5bff3f0a-d89a-421f-aa75-1864dca094ab" />

This repo contains the scripts and data for EAS 501: Sustainable Urban Systems.

The outcome of these tutorials:
- Working in base Python
- Data cleaning and analysis in Python using Pandas
- Mapping in Python using GeoPandas

Just a few of my favorite maps output from these tutorials:

<img width="504" height="414" alt="Screenshot 2026-04-02 at 12 49 19 PM" src="https://github.com/user-attachments/assets/7fbe8c4e-38f3-4d40-9ca2-67669595c85d" />

<img width="504" height="414" alt="Screenshot 2026-04-02 at 12 47 22 PM" src="https://github.com/user-attachments/assets/3418586b-4d42-4468-9c6f-a09543c531e8" />
